...
class RunAudit:
    def __init__(
        ...
        if self.run_dir.exists():
            ...
            if not resolved_run_dir.is_relative_to(runs_root) or resolved_run_dir == runs_root:
                raise ValueError(f"Refusing to clear unsafe run directory: {resolved_run_dir}")
        ...
        self.success_records_dir.mkdir(parents=True, exist_ok=True)
    ...
    def should_audit_cv(self, index: int, has_review_flags: bool, failed: bool) -> bool:
    ...
    def write_cv_audit(self, cv_id: str, index: int, record: dict[str, Any]) -> Path:
        filename = f"{index:04d}_{_safe_filename(cv_id)}.json"
    ...
    def write_failed_record(self, cv_id: str, index: int, failed_case: dict[str, Any]) -> Path:
        filename = f"{index:04d}_{_safe_filename(cv_id)}.json"
